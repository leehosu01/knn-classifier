import warnings
import torch
def __predict(test_X:torch.tensor, train_X:torch.tensor, train_Y:torch.tensor, knn_k:int, knn_t:float):
    ...
    return pred_label
def predict(test_X, train_X, train_Y, knn_k:int = 200, knn_t:float = 0.1, device = None):
    ...
    return __predict(test_X, train_X, train_Y, knn_k, knn_t)